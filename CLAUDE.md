...
`opts.inline` returns an entirely different catalog (`inlineCatalog`), built before any database or watcher exists: queries walk the collection and run `process()` in memory. It reproduces stored-index results rather than approximating them, by keying entries with the same `charwise.encode([...emittedKey, path])` a stored index writes — charwise encodings compare lexicographically in charwise order, so the same bounds and sentinels give the same ordering and subtree semantics. It reads only (creates neither directory), `close()`/`reindex()` have nothing to do, `indexPath` is `undefined`, and `'problem'`/`'resolved'` are not emitted since there is no stored state to transition. Live/inline parity is pinned by a loop in the test file that runs the same assertions against both modes.
...
- `opts.dataPath` is `mkdirSync`'d before watching because chokidar won't reliably see files created in a directory that didn't exist when the watch began. `opts.indexPath` is created eagerly too, before any `ClassicLevel` is constructed: classic-level would create it lazily on first use, but then an unwritable location surfaces as an opaque `LEVEL_ITERATOR_NOT_OPEN` on the first query instead of an attributable EACCES at construction.

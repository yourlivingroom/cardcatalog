...
export default function cardcatalog(indexes, opts = {}) {
    ...
    const indexDbs = Object.fromEntries(Object.entries(indexes).map(
        ([k, v]) => [
            k,
    ...
    async function updatePath(path, remove, stats) {
        const fileContent =
                remove ? undefined : await fs.promises.readFile(path);

        for (const k of Object.keys(indexes)) {
            const indexConfig = indexes[k];

            const fileMeta =
                    (await indexDbs[k].sublevel('fileMeta').get(
                            path, { valueEncoding: 'json' }))
                    ?? {};
    ...
    function queueRemove(path) {
        ...
        queue.add(() => updatePath(path, true))
                .catch(e => console.error(
                        'cardcatalog remove failed for ' + path + ':', e));
    }

    function queueUpdate(path, stats) {
        if (!opts.shouldIndex(path, stats)) return;
        debug('watcher: add/change', path);
        queue.add(() => updatePath(path, false, stats))
                .catch(e => console.error(
                        'cardcatalog update failed for ' + path + ':', e));
    }
    ...
    const watcher = chokidar.watch(opts.dataPath)
    .on('add', queueUpdate)
    .on('change', queueUpdate)
    .on('unlink', queueRemove);

    return {
        catalogs: Object.fromEntries(Object.entries(indexDbs).map(
            ([indexName, levelDb]) => [
                indexName,
                {
                    async get(key) {
